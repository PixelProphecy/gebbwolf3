...
The magic happens in function \cw{VL\_DePlaneVGA} where VGA registers are manipulated to tweak what the BIOS had set up in Mode 13h. \\
...
\end{minipage}
 ...
 \par
 The VGA registers of the Sequence Controller and CRT Controller are set up to divide the 256KiB VRAM into four parts.
...
\centering
 ...
 \end{figure}
\par
This glitch shows both misalignment and parts of two pages. This problem has to do with atomicity. The CRTC starting address is a 16-bit value but the \cw{out} instruction can only write 8 bits at a time. If the pages are set up one after another like this.
\par
\begin{figure}[H]
 \centering
